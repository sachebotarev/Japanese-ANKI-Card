...
def main() -> None:
    ...
    if (ROOT / "Карточки" / "Покупки и сервис").exists():
        rename_theme_folder("Покупки и сервис", "Торговля")

    if (ROOT / "Карточки" / "Учёба").exists():
        rename_theme_folder("Учёба", "Образование")
    ...
    for w in ("パン", "卵", "果物", "肉", "野菜", "魚"):
        move_word_bundle(w, "Повседневность", "Еда")
    for w in ("サッカー", "テニス"):
        move_word_bundle(w, "Повседневность", "Спорт")
    move_word_bundle("花見", "Повседневность", "Традиции")
    move_word_bundle("写真", "Повседневность", "Фотография")
    move_word_bundle("たばこ", "Повседневность", "Торговля")
    move_word_bundle("期待", "Повседневность", "Эмоции")

    # 6) Из «Существительные»
    move_word_bundle("二足", "Существительные", "Счётные суффиксы")
    move_word_bundle("耳鼻科", "Существительные", "Медицина")
    move_word_bundle("石油", "Существительные", "Природа")
    move_word_bundle("大阪城", "Существительные", "Туризм")
    ...
    move_word_bundle("日常系", "Существительные", "Кинематограф")
    for w in ("犯人", "持ち主", "場合"):
        move_word_bundle(w, "Существительные", "Право")
